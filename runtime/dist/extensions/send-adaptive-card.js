...
const SendAdaptiveCardSchema = Type.Object({
    ...
    submit_behavior: Type.Optional(Type.Union([
...
export const sendAdaptiveCard = (pi) => {
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params) {
            ...
            if (!content)
                return buildResultError("Provide content.");
            const cardId = params.card_id?.trim();
            if (!cardId)
                return buildResultError("Provide card_id.");
            ...
            if (params.submit_behavior)
                block.submit_behavior = params.submit_behavior;
