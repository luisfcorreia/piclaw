...
describe("audit-baseline-quality-deterministic", () => {
  ...
  test("list-groups exposes finer deterministic subgroup coverage", async () => {
    ...
    expect(lines).toContain("channels-web-http-routes\t33\tchannels web http and route surfaces");
    expect(lines).toContain("channels-web-media-workspace-remote\t16\tchannels web media, workspace, and remote surfaces");
    expect(lines).toContain("web-ui-interaction-and-state\t79\tweb ui interaction and state");
    expect(lines).toContain("web-ui-rendering-and-panes\t62\tweb ui rendering and panes");
