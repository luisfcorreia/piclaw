...
type InstalledAddonExtensionEntry = {
  packageName: string;
  packageDir: string;
  relativePath: string;
  fullPath: string;
...
const ADDON_EXTENSION_BRIDGE_PREFIX = "addon-bridge--";
...
export function getInstalledAddonExtensionEntries(workspaceDir = getWorkspaceDir()): InstalledAddonExtensionEntry[] {
  const addonsNodeModulesDir = join(workspaceDir, ".pi", "addons", "node_modules");
  const entries: InstalledAddonExtensionEntry[] = [];
  for (const packageDir of listAddonPackageDirs(addonsNodeModulesDir)) {
    ...
    try {
      ...
      const packageName = typeof manifest?.name === "string" && manifest.name.trim() ? manifest.name.trim() : packageDir.split("/").pop() || "addon";
      ...
      for (const relativePath of declared) {
        const normalizedRelativePath = String(relativePath || "").replace(/^\.\//, "").trim();
        if (!normalizedRelativePath) continue;
        const fullPath = join(packageDir, normalizedRelativePath);
...
export function getInstalledAddonExtensionPaths(workspaceDir = getWorkspaceDir()): string[] {
  return getInstalledAddonExtensionEntries(workspaceDir).map((entry) => entry.fullPath);
}

function sanitizeAddonBridgeName(value: string): string {
  return value.replace(/[^a-zA-Z0-9._-]+/g, "-");
}

export function syncInstalledAddonExtensionBridges(workspaceDir = getWorkspaceDir()): string[] {
  ...
  for (const entry of readdirSync(workspaceExtensionsDir, { withFileTypes: true })) {
    ...
    if (!entry.name.startsWith(ADDON_EXTENSION_BRIDGE_PREFIX)) continue;
    rmSync(join(workspaceExtensionsDir, entry.name), { force: true });
  }

  const bridgePaths: string[] = [];
  const entries = getInstalledAddonExtensionEntries(workspaceDir);
  entries.forEach((entry, index) => {
    const bridgeName = `${ADDON_EXTENSION_BRIDGE_PREFIX}${sanitizeAddonBridgeName(entry.packageName)}--${index}.ts`;
    const bridgePath = join(workspaceExtensionsDir, bridgeName);
    const importPath = relative(workspaceExtensionsDir, entry.fullPath).replace(/\\/g, "/");
    writeFileSync(bridgePath, [
      "// Auto-generated by piclaw add-on install sync. Do not edit.",
...
export async function createSessionInDir(
  ...
  const workspaceDir = getWorkspaceDir();
  syncInstalledAddonExtensionBridges(workspaceDir);
