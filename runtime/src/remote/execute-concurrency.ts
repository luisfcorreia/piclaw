...
export class RemoteExecuteConcurrency {
  private readonly inflightByPeer = new Map<string, number>();
  ...
  isAtLimit(peerInstanceId: string): boolean {
  ...
  acquire(peerInstanceId: string): void {
  ...
  release(peerInstanceId: string): void {
    this.inflightTotal = Math.max(0, this.inflightTotal - 1);
    const count = (this.inflightByPeer.get(peerInstanceId) || 1) - 1;
    if (count <= 0) this.inflightByPeer.delete(peerInstanceId);
    else this.inflightByPeer.set(peerInstanceId, count);
  }
}
