...
import { logRemoteAudit, type RemotePeerRecord } from "../db/remote-interop.js";
...
export function buildCallbackProofString(requestId: string, challenge: string, receiverInstanceId: string): string {
  return `${requestId}\n${challenge}\n${receiverInstanceId}`;
}

/** Verify a remote peer callback proof before establishing trust. */
export async function verifyCallbackProof(
  pending: {
    id: string;
    nonce: string | null;
...
export function logAudit(peer: RemotePeerRecord | null, endpoint: string, status: string, decision?: string, error?: string): void {
  logRemoteAudit({
