...
const QRErrorCorrectLevel = { L: 1, M: 0, Q: 3, H: 2 } as const;
const QRMode = {
  MODE_NUMBER: 1 << 0,
  MODE_ALPHA_NUM: 1 << 1,
  MODE_8BIT_BYTE: 1 << 2,
...
const QR_LENGTH_IN_BITS: Record<number, [number, number, number]> = {
...
const QRUtil = {
  ...
  getLengthInBits(mode: number, type: number) {
    ...
    if (!values) throw new Error(`mode:${mode}`);
    if (1 <= type && type < 10) return values[0];
    if (type < 27) return values[1];
    if (type < 41) return values[2];
...
class QRCode {
  ...
  modules: Array<Array<boolean | null>> | null = null;
  moduleCount = 0;
...
export function buildQrMatrix(data: string, level: QrLevel): boolean[][] {
