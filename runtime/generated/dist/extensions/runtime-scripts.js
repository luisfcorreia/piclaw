...
import { normalizeScriptJDoc } from "./discovery-jdoc.js";
...
function collectExampleStrings(metadata) {
...
function cleanCommentLines(value) {
    return value
        .split(/\r?\n/)
...
export function extractScriptJDocJson(sourceText) {
...
export function parseScriptJDocFromSource(sourceText) {
...
function detectRole(absolutePath, sourceText, metadata) {
    if (metadata?.role)
        return metadata.role;
    const tokens = absolutePath.toLowerCase().split(/[^a-z0-9]+/).filter(Boolean);
    if (tokens.includes("helper") || tokens.includes("helpers") || tokens.includes("sidecar"))
        return "module";
    if (tokens.includes("scope") && tokens.includes("session"))
        return "module";
    if (/^#!\/.+/m.test(sourceText) || /process\.argv|Bun\.argv|parseArgs\s*\(/.test(sourceText))
        return "entrypoint";
    if (absolutePath.includes(`${path.sep}runtime${path.sep}skills${path.sep}`))
        return "entrypoint";
    if (absolutePath.includes(`${path.sep}skills${path.sep}`) && absolutePath.includes(`${path.sep}runtime${path.sep}extensions${path.sep}`))
        return "entrypoint";
    if (absolutePath.startsWith(path.join(WORKSPACE_DIR, ".pi", "skills") + path.sep))
        return "entrypoint";
    if (absolutePath.startsWith(path.join(WORKSPACE_DIR, "notes") + path.sep))
        return "entrypoint";
    return "module";
}
function fallbackSummary(absolutePath, collection, role) {
...
export function getScriptRoots(workspaceDir = WORKSPACE_DIR) {
...
function buildInvocation(absolutePath, workspacePath) {
    if (workspacePath) {
        return {
            runner: "bun",
...
export function loadScriptCatalogEntries(options) {
    ...
    for (const root of getScriptRoots(workspaceDir)) {
        if (scope !== "all" && root.scope !== scope)
            continue;
        for (const absolutePath of walkTsFiles(root.root)) {
            if (!root.include(absolutePath))
                continue;
...
function scoreIntent(entry, intent) {
