...
export async function handleState(session, _command) {
    ...
    if (isOversizedSession && sessionFileSize !== null) {
        lines.push(`Session file warning: exceeds configured threshold (${formatBytes(sessionFileSize)} >= ${sessionStorageConfig.maxSizeMb} MB). Consider /session-rotate.`);
    }
...
export async function handleCommands(session, _command) {
    const describeSource = (source, detail, scope) => {
    ...
    const addEntry = (name, description, source, detail, scope) => {
