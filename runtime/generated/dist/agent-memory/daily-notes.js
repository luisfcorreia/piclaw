...
function stripLegacyWatermark(body) {
...
function appendSummaryUpdate(body, lastTs) {
    if (body.includes(SUMMARY_UPDATE_MARKER))
        return body;
...
export function refreshDailyNotesFromMessages(options) {
    ...
    const rows = db.query(`SELECT m.sender_name,
            m.is_bot_message,
            m.content,
            m.timestamp,
            m.chat_jid,
     ...
     ORDER BY m.timestamp ASC`).all(...params, cutoff);
